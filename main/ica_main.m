...
%%程序选择
function ica_main(data_path,save_path)
    global GDS;
    global GDS_Name;
    global GLearners;
    global GFS;
    ...
    fs_fun(data_path,save_path,GDS([1,2,3,4,5,6,7,8]),GFS([4,6,7]),GLearners([2]),[100,150]);
end
...
function [dcplx,ica] = ECOC_train(DName,TD,TL,TTD,TTL,classifier)
    ...
    dcplx={};
    [data_test_label,all_labels,accuaryratio,elapsed,tcplx,tcds,confmat]=ica_ecoc(TD,TL,TTD,TTL,'data_cplx',classifier);
...
function fs_fun(data_path,save_path,DS,FS,Learners,feature_size)
    global index;
    global h;
    learnerdata={};
    fm=size(FS,2)*size(DS,2)*size(Learners,2)*size(feature_size,2);
        ...
        for fs_option = 1: size(FS,2) %feature selection        
            ...
            for dataset_option = 1: size(DS,2) %dataset
                ...
                for lindex = 1: size(Learners,2) %base learner%
                    ...
                    %获取原始数据
                     [TD,TL,TTD,TTL] = load_data([data_path,'/data_original/'],DS{dataset_option});
                     index=index+10/fm;
                     waitbar(index/100,h,['已完成 ',num2str(round(index)),'%']);
 
                     %获取特征选择数据
                     [FS_TD,FS_TTD] = get_fsdata([data_path,'/data_fs/',DS{dataset_option},'/',DS{dataset_option},'_',FS{fs_option},'.mat'],TD,TTD,feature_option);
                     disp(['加载数据：',data_path,'/data_original/',DS{dataset_option}]);
                     index=index+15/fm;
                     waitbar(index/100,h,['已完成 ',num2str(round(index)),'%']);
...
%                    load(['C:\Users\yangyr\Desktop\ICDC-ECOC\ICDC-ECOC\data\data_fs_10--150_pair_newN3_newF3\data_fs_100\',FS{fs_option},'\ica\ica_',Learners{lindex},'_',DS{dataset_option}]);
%                    load(['C:\Users\yangyr\Desktop\ICDC-ECOC\ICDC-ECOC\data\data_fs_10--150_pair_newN3_newF3\data_fs_100\',FS{fs_option},'\dcplx\dcplx_',Learners{lindex},'_',DS{dataset_option}]);
                    ...
                    respath = [save_path,'/data_fs_',num2str(feature_option),'/',FS{fs_option}];
                    mkdir(respath);
...
function draw_size_change(genpath,DS,FS,Learners,feature_size) 
    ...
    subnames = {'4-F1-F3';'4-N2-N3';'4-N4-L3';'4-Cluster';'6-F1-F3';'6-N2-N3';'6-N4-L3';'6-Cluster';'ALLDC';'3-F1-F3';'3-N2-N3';'3-N4-L3';'3-Cluster';'3-N_C1'};
        ...
        for ecoc_option = 14
            ...
            respath1 = [genpath,'/pic/期刊1/new_N3_F3/fs_size/',subnames{ecoc_option}];
            mkdir(respath1);

            for lindex = 3

                for dataset_option = 1 
                    ...
                    for fs_option = 1:3
                        ...
                        for i =5:5:150 % feature size
                            ...
                            %%获取dcplx MAT 文件
                            dcplxmat = [genpath,'/data_fs_10--150_pair_newN3_newF3/data_fs_',num2str(i),'/',fs_method{fs_option},'/','dcplx/dcplx_',learners{lindex},'_',DS{dataset_option}];
                            icamat= [genpath,'/data_fs_10--150_pair_newN3_oldF3/data_fs_',num2str(i),'/',fs_method{fs_option},'/','ica/ica_',learners{lindex},'_',DS{dataset_option}];
                            load(dcplxmat);
                            load(icamat);
                    ...
                    saveas(gcf ,[respath1,'/',learners{lindex},'_',DS{dataset_option},'_',subnames{ecoc_option}],'fig');
                    saveas(gcf ,[respath1,'/',learners{lindex},'_',DS{dataset_option},'_',subnames{ecoc_option}],'tiff');
...
function draw_acc_cplx_fun(genpath,DS,FS,Learners,feature_size)   
    ...
    for num = feature_size
        ...
        for fs_option = 2
            ...
            for ecoc_option =size(ecocnames,2)
                ...
                for lindex = 3
                    ...
                    for data_option=1:size(DS,2)
                            ...
                            accuracy = dcplx{data_option,2}{1,ecoc_option}(1);
                            allcplx = dcplx{data_option,6}{1,ecoc_option};
                            cplxsum = 0;
...
function draw_PD(genpath,DS,FS,feature_size)
    picrespath = [genpath,'\pic\期刊\new_N3_F3\ecoc_PD'];
    mkdir(picrespath);
    ...
    for num = feature_size
        ...
        for fs_option = 1:size(FS,2)
            ...
            for data_option=1:size(DS,2)
                ...
                %加载复杂度矩阵
                dcplxmat = [genpath,'/data_fs_10--150_pair_newN3_newF3/data_fs_',num2str(num),'/',FS{fs_option},'/','dcplx/dcplx_svm_',DS{data_option}];
                ...
                for  ci = 1:ecoc_num
                    for cj = ci+1:ecoc_num
                        dcplx_PD_matrix(ci,cj) = get_PD(dcplx_ecocs{ci},dcplx_ecocs{cj});       
                    end
                end

                %加载其他算法的矩阵
                icamat = [genpath,'/data_fs_10--150_pair_newN3_newF3/data_fs_',num2str(num),'/',FS{fs_option},'/','ica/ica_svm_',DS{data_option}];
                disp(['加载的dcplx是：',num2str(num),'/',FS{fs_option},'/','ica/ica_svm_',DS{data_option}]);
                load(icamat);            
                ica_ecocs = ica{data_option,7};
                ica_ecocs(1) = [];
                ica_ecocs(4) = [];
                ...
                fileresname = [resfilepath,'\',FS{fs_option},'_',DS{data_option},'_dcplx_PD.csv'];        
